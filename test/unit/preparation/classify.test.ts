...
describe('containment classification', () => {
  it('maps depth 0/1/2/3 to outer/hole/island/hole', async () => {
  ...
  it('detects points inside and outside a ring', async () => {
    assert.equal(pointInRing([5, 5], rect(0, 0, 10, 10)), true)
    assert.equal(pointInRing([50, 50], rect(0, 0, 10, 10)), false)
  })

  it('returns an interior sample point, not the raw centroid', async () => {
