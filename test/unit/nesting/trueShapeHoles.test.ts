...
describe('trueShape/holes', () => {
  ...
  it('hosts a part inside the hole when it cannot fit outside (FR-001/FR-002)', () => {
    const part = shapeFrom('p1', closed([[0, 0], [20, 0], [20, 20], [0, 20], [0, 0]]))
    const parts: PartRequest[] = [{ shape: part, quantity: 1 }]
    const r = nestTrueShape({ stock: [sheet], parts, edgeClearance: 2, partToPartClearance: 2, seed: 1 })
  ...
  it('reports a part larger than the hole and the frame as unplaced', () => {
