...
describe('trueShape/rotações + grão (FR-004)', () => {
  ...
  it('grainLocked + grainAngle 0 only uses orientations aligned with the grain', () => {
    const parts: PartRequest[] = [{ shape, quantity: 2, grainLocked: true, grainAngle: 0 }]
    const r = nestTrueShape({ stock, parts, edgeClearance: 2, partToPartClearance: 2, seed: 7 })
    expect(r.unplaced).toHaveLength(0)
    expect(r.placements).toHaveLength(2)
    for (const p of r.placements) expect([0, 180]).toContain(p.rotation)
  })

  it('grainLocked without grainAngle is reported unplaced, never an arbitrary orientation', () => {
  ...
  it('never scales: the placed bounding box matches a pure rotation of the source shape', () => {
