...
## Visão geral
Módulo `config.ts` da feature 002-true-shape-nesting.

## Responsabilidades
- Constantes `DEFAULT_EDGE_CLEARANCE`, `DEFAULT_PART_TO_PART_CLEARANCE`, `DEFAULT_SEARCH_BUDGET_FACTOR`.

## Entradas / Saídas
- Entradas: parâmetros tipados das funções exportadas em `config.ts`.
