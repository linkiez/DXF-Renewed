...
function entityToDxfTuples(entity: Entity): string[] {
	...
	switch (entity.type) {
		case 'LINE': {
			...
			tuples.push(
				'10', String(line.start?.x ?? 0),
				'20', String(line.start?.y ?? 0),
				'11', String(line.end?.x ?? 0),
				'21', String(line.end?.y ?? 0),
			)
			break
		}
		...
		case 'CIRCLE': {
			...
			tuples.push(
				'10', String(circle.x ?? 0),
				'20', String(circle.y ?? 0),
				'40', String(circle.r ?? 0),
			)
			break
		}
