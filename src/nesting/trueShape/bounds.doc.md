...
## Visão geral
Módulo `bounds.ts` da feature 002-true-shape-nesting.

## Responsabilidades
- Contenção (FR-001): dentro da chapa e a `edgeClearance` de todo contorno, incluindo furos; expõe `polygonDistance`.

## Entradas / Saídas
- Entradas: parâmetros tipados das funções exportadas em `bounds.ts`.
