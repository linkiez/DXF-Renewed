...
### Session 2026-09-12 (analysis remediation)

- Q: How is a remnant that is "too small" identified? (Edge Cases) → A: The caller supplies `remnantThreshold`, a minimum remnant bounding-box area in the squared input unit; any `kind: 'remnant'` stock item below it is excluded before the search. No implicit project-wide default is applied. → FR-008.
- Q: Where does a grain-locked part's grain direction come from? (FR-004) → A: Each part request declares `grainAngle` in degrees; a grain-locked part is restricted to the subset of `allowedRotations` aligned to `grainAngle` modulo 180°, and a grain-locked part with no `grainAngle` is reported in `unplaced` with an explicit reason rather than placed arbitrarily.
...
- Grain-locked parts only use the allowed orientations that are aligned with their
  grain direction.
- A remnant whose bounding-box area is below the caller-supplied `remnantThreshold` is excluded
  before search (FR-008).
...
- **FR-004**: The system MUST rotate parts only to orientations in the caller-supplied
  permitted rotation list, defaulting to 0/90/180/270 when the list is omitted; parts marked
  grain-locked MUST be restricted to the subset of those orientations aligned with the part's
  caller-declared grain angle (`grainAngle`, aligned modulo 180°). A grain-locked part with no
  declared grain angle MUST be returned in `unplaced` with an explicit reason rather than placed
  at an arbitrary orientation.
- **FR-005**: The system MUST select stock items and placements so as to maximize total
  material use across the job, evaluating alternative placements rather than committing to a
  per-part greedy choice; arrangements that use the same amount of material MUST be resolved in a
  stable, deterministic order. The search MUST be bounded by a deterministic budget of iterations
  or nodes derived from the input size, MUST NOT depend on wall-clock time, and MUST be calibrated
  so that a 100-part job returns within 2 seconds.
- **FR-006**: The system MUST report sheet use, material use — placed part area divided by
  consumed stock area — waste, and all unplaced quantities.
- **FR-007**: The system MUST make identical placement decisions for identical inputs and seed.
- **FR-008**: The system MUST exclude every `kind: 'remnant'` stock item whose bounding-box area
  is below the caller-supplied `remnantThreshold` before the search begins, and MUST NOT apply an
  implicit default threshold when the caller omits one.
...
- **SC-001**: Every returned layout contains zero overlaps and zero out-of-bound placements.
...
- The caller supplies `remnantThreshold` (minimum remnant bounding-box area) and, for grain-locked
  parts, `grainAngle`; the engine applies no implicit default for either.
